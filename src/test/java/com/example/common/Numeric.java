...
public class Numeric {
    ...
    public static String toHexStringNoPrefix(BigInteger value, int length) {
        return toHexStringNoPrefix(toBytesPadded(value, length));
    }
    ...
    public static byte[] hexStringToByteArrayZeroPadded(String input, Integer length) {
        byte[] b = hexStringToByteArray(input);
        if (b.length < length) {
            byte[] tmpBytes = new byte[length];
            System.arraycopy(b, 0, tmpBytes, length - b.length, b.length);
            b = tmpBytes;
        }
        return b;
    }
    ...
    public static boolean isHex(String data) {
        ...
        return pattern.matcher(cleanHexPrefix(data).toLowerCase()).matches();
    }
